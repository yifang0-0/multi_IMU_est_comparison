...
from GlobalParams import params # Global parameters object
...
def setup_global_params(config):
    """Sets necessary values in the global 'params' object."""
    params.Fs = config['FS']
    params.T = 1 / config['FS']
    params.iteration = config['ITERATION']
    params.cov_lnk = np.eye(3) * config['CW_LNK']
    params.cov_w = np.eye(6) * config['CW_W']
    print(f"Global Params set: Fs={params.Fs}, T={params.T}, Cov_lnk={config['CW_LNK']}")
