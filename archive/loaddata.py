...
from constants import FS, T, N_STILL
...
def _generate_time_array(data, N):
    """Generates a time array based on N and T if 't' is missing."""
...
def _run_calculations(data):
    ...
    gyr1_still = gyr_1[:, :N_STILL]
    gyr2_still = gyr_2[:, :N_STILL]

    sdNoise_gyr1 = np.std(gyr1_still, ddof=1, axis=1)
...
def data_transmat2npz(data_path, FORCE_REPROCESS=False):
    ...
    if needs_processing:
        ...
        data, N = _run_calculations(data)
        data['t'] = _generate_time_array(data, N)
...
def data_load_process(TRUNCATE, data_path, FORCE_REPROCESS=False, verbose=True):
    ...
    Returns:
        data: Dictionary with processed sensor data, or None if loading fails.
    """
